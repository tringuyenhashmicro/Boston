...
class contactus(http.Controller):
    ...
    def get_contactus_response(self, values, kwargs):
        ...
        return request.website.render(kwargs.get("view_callback", "boston_modifier_status.contactus_thanks"), values)
    ...
    def contactus(self, **kwargs):
        ...
        if lead_id:
            ...
            lead_info = request.registry['crm.lead'].browse(request.cr, SUPERUSER_ID, lead_id)
            partner_id = request.registry['res.partner'].create(request.cr, SUPERUSER_ID, {'name': lead_info.contact_name,
                                                                                           'email': lead_info.email_from,
                                                                                           'phone': lead_info.phone})
            request.registry['crm.lead'].write(request.cr, SUPERUSER_ID, [lead_id], {'partner_id': partner_id})
            account_id = request.registry['res.partner'].browse(request.cr, SUPERUSER_ID, partner_id).property_account_receivable.id
            journal_id = request.registry['account.journal'].search(request.cr, SUPERUSER_ID, [('type', '=', 'sale')])
            journal_id = journal_id and journal_id[0] or 0
            request.registry['account.invoice'].create(request.cr, SUPERUSER_ID, {'partner_id': partner_id, 
